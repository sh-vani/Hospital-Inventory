  ...
  const handleSaveEdit = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        alert(`Item GHS{currentItem.item_code} updated successfully`);
  ...
  const handleBulkAdd = async () => {
    try {
      ...
      setTimeout(() => {
        ...
        alert(`GHS{bulkItems.length} items added successfully`);
    ...
    return (
      <nav className="d-flex justify-content-center mt-3">
        <ul className="pagination mb-0">
          <li className={`page-item GHS{currentPage === 1 ? "disabled" : ""}`}>
            <button
              className="page-link"
          ...
          {pageNumbers.map((number) => (
            <li
              key={number}
              className={`page-item GHS{number === currentPage ? "active" : ""}`}
            >
